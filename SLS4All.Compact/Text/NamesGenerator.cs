...
{
    /// <summary>
    /// Generates random names. Modified version of https://raw.githubusercontent.com/moby/moby/master/pkg/namesgenerator/names-generator.go
    /// </summary>
    public static class NamesGenerator
    {
        private readonly static string[] _left = new[]
        {
            "admiring",
            "adoring",
            "affectionate",
		...
		private static readonly string[] _right =
		{
			...
			// Dame Stefanie "Steve" Shirley - Founded a software company in 1962 employing women working from home. https://en.wikipedia.org/wiki/Steve_Shirley
			"shirley",
		...
		public static string GenerateRandomName(bool capitalize, bool identifier)
		{
			string left, right;
			lock (s_random)
			{
				left = _left[s_random.Next(_left.Length)];
                right = _right[s_random.Next(_left.Length)];
            }
            var buf = new StringBuilder(left.Length + right.Length + 1);
            if (capitalize)
			{
				buf.Append(char.ToUpper(left[0]));
				buf.Append(left, 1, left.Length - 1);
            }
			else
			{
                buf.Append(left);
			...
			if (capitalize)
			{
				buf.Append(char.ToUpper(right[0]));
				buf.Append(right, 1, right.Length - 1);
			}
			else
			{
				buf.Append(right);
